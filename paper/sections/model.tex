...
I use a quantitative spatial model based on \cite{monte_commuting_2018} and \cite{ahlfeldt_economics_2015}. The key idea is that workers choose both where to live and where to work, and markets clear. I simplify by ignoring goods trade across locations due to both a lack of data and a lack of meaningful trade costs within the city. 
I focus on commuting costs as the key friction that the transit extension affects.

\subsection{Setup}

There are $N$ locations, which I will calibrate to census tracts in Chicago. Workers choose a residence $n$ and a workplace $i$. Each location has a fixed housing supply $H_n^R$ with price $Q_n$. There's a consumption good that's freely traded and serves as numeraire.
...
\end{equation}
Here $B_n$ is a residential amenity, $\kappa_{ni} \geq 1$ is a commuting cost from $n$ to $i$, and $b_{ni\omega}$ is an idiosyncratic taste shock. The key assumption is that these taste shocks follow a Fr\'{e}chet distribution with shape $\varepsilon > 1$, as usual.
...
$\kappa_{ni}$ is an iceberg commuting cost between locations $n$ and $i$.
...
\end{equation}
When $\gamma < 1$, wages fall if more workers enter the location, allowing a congestion force to pin down a stable equilibrium.
...
\item Recover wages by finding $\{w_i\}$ that make the model's predicted employment match observed employment, given commuting costs.
\item Recover floor prices from land market clearing: $Q_n = (1-\alpha) \bar{w}_n L_n^R / H_n^R$.
\item Recover amenities from residential shares: $B_n^\varepsilon \propto \lambda_n^R Q_n^{(1-\alpha)\varepsilon} / \widetilde{W}_n$.
\item Recover productivity from the wage equation: $A_i = w_i / (\gamma (L_i^M)^{\gamma-1})$.
\end{itemize}

\subsection{Counterfactual: Solving in Changes}

For the counterfactual, I use exact hat algebra following \cite{dekle_unbalanced_2007,dekle_global_2008}. Let hats denote ratios: $\hat{x} = x'/x$ where primes are counterfactual values. Given changes in commuting costs $\hat{\kappa}_{ni}^{-\varepsilon} = \exp(-\nu(\tau'_{ni} - \tau_{ni}))$, I solve for changes in all endogenous variables.
